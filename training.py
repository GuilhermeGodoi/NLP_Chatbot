...
model = Sequential()
model.add(Dense(256, input_shape=(len(train_x[0]),), activation='relu')) #Input layer = dense layer with 128 neurons. The same goes for future codes (Not necessarily for input)
...
hist = model.fit(np.array(train_x), np.array(train_y), epochs=300, batch_size=5, verbose=1) #epochs = feat same data 200 times in the NeuralNetwork
